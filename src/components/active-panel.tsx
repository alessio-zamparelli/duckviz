...
import { activePanelAtom, isLoadingAtom, type panelType } from '@/atoms/state'
    ...
    return (
      <div className='mx-px flex flex-col justify-center gap-2 px-1 shadow-sm'>
        <Button
          ...
          onClick={() => {
            if (position === 'top') {
              setActivePanel(s => ({ top: 'query', bottom: s.bottom === 'query' ? s.top : s.bottom }))
            } else {
              setActivePanel(s => ({ bottom: 'query', top: s.bottom === 'query' ? s.bottom : s.top }))
            }
          }}>
          <DatabaseIcon />
        </Button>
        ...
        <Button
          ...
          onClick={() => {
            if (position === 'top') {
              setActivePanel(s => ({ top: 'plot', bottom: s.bottom === 'plot' ? s.top : s.bottom }))
            } else {
              setActivePanel(s => ({ bottom: 'plot', top: s.bottom === 'plot' ? s.bottom : s.top }))
            }
          }}>
          <ChartLineIcon />
        </Button>
